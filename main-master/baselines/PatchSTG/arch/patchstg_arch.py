...
class PatchSTG(nn.Module):
    def __init__(self, tem_patchsize, tem_patchnum, output_len,
    ...
    # def forward(self, x, te):
        ...
        # print(f"pred_y shape: {pred_y.shape}")  # Debugging line to check output shape
    ...
    def embedding(self, x, te):
        ...
        x1 = torch.cat([x,te[...,0:1],te[...,1:2]], -1).float()
        ...
        t_i_d_data = (te[:, -input_data.shape[1]:, :, 0] * self.tod).type(torch.LongTensor)
        ...
        d_i_w_data = (te[:, -input_data.shape[1]:, :, 1] * self.dow).type(torch.LongTensor)
