...
def get_splitter(dataset_name):
    if dataset_name == "la":
        return AtTimeStepSplitter(
            first_val_ts=(2012, 5, 25, 16, 00),
            last_val_ts=(2012, 6, 4, 3, 20),
            first_test_ts=(2012, 6, 4, 4, 20),
        )
    elif dataset_name == "sd":
        return MyTimeStepSplitter(
            input_len=96,
            output_len=192,
            data_file_path="datasets/sd",
            first_val_ts=(2016, 8, 7, 4, 40),
            last_val_ts=(2016, 8, 19, 13, 55),
            first_test_ts=(2016, 8, 19, 13, 55),
            last_test_ts=(2016, 8, 31, 23, 55),
        )
    elif dataset_name == "bay":
        return AtTimeStepSplitter(
            first_val_ts=(2017, 5, 11, 7, 20),
...
def configure_parser():
    ...
    parser.add_argument("--dataset-name", type=str, default="sd")
    parser.add_argument("--config", type=str, default="traffic/sgp_sd_96_192.yaml")

    # preprocessing options
    parser.add_argument(
        "--preprocess-exogenous", type=str_to_bool, nargs="?", const=True, default=False
    )
    parser.add_argument(
        "--keep-raw", type=str_to_bool, nargs="?", const=True, default=True
    )
    ...
    parser.add_argument(
        "--iid-sampling", type=str_to_bool, nargs="?", const=True, default=False
    )
    parser.add_argument(
        "--sgp-preprocessing", type=str_to_bool, nargs="?", const=True, default=False
    )
...
def run_experiment(args):
    ...
    logdir = os.path.join(
        lib.config["logs_dir"], args.dataset_name, args.model_name, exp_name
    )
    ...
    adj = dataset.get_connectivity(
        threshold=0.1, layout="edge_index", include_self=False
    )
    ...
    if args.sgp_preprocessing or args.iid_sampling:
        dm = SGPDataModule(
            dm,
            iid_sampling=args.iid_sampling,
...
def set_random_seed(seed=42):
    ...
    # If using distributed training, ensure deterministic seed for each process
