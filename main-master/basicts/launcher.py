...
def evaluation_func(
    ...
    runner.init_logger(
        logger_name="easytorch-evaluation", log_file_name="evaluation_log"
    )
    ...
    try:
        ...
        if batch_size is not None:
            ...
            assert (
                "BATCH_SIZE" in cfg.TEST.DATA
            ), "Batch size must be specified either in the config or as an argument."

        # load the model checkpoint
        if ckpt_path is None or not os.path.exists(ckpt_path):
            ckpt_path_auto = os.path.join(
                runner.ckpt_save_dir,
                "{}_best_val_{}.pt".format(
                    runner.model_name, runner.target_metrics.replace("/", "_")
                ),
            )
            logger.info(
                f"Checkpoint file not found at {ckpt_path}. Loading the best model checkpoint `{ckpt_path_auto}` automatically."
            )
...
def launch_training(
    cfg: Union[Dict, str], gpus: Optional[str] = None, node_rank: int = 0
) -> None:
    """
