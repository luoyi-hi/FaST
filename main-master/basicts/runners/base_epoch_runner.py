...
class BaseEpochRunner(metaclass=ABCMeta):
    ...
    def __init__(self, cfg: Dict) -> None:
        ...
        self.early_stopping_patience = cfg.get("TRAIN", {}).get(
    ...
    def build_model(self, cfg: Dict) -> nn.Module:
        ...
        if cfg.get("TRAIN.COMPILE_MODEL", False):
            ...
            if version.parse(current_version) >= version.parse("2.0"):
                ...
                self.logger.warning(
                    f"torch.compile requires PyTorch 2.0 or higher. Current version: {current_version}. Skipping compilation."
                )
    ...
    def build_lr_scheduler(self, cfg: Dict) -> None:
        ...
        if cfg.has("TRAIN.LR_SCHEDULER"):
            self.scheduler = optim.build_lr_scheduler(
    ...
    def init_training(self, cfg: Dict) -> None:
        ...
        if cfg.has("TRAIN.FINETUNE_FROM"):
            self.load_model(
                cfg["TRAIN.FINETUNE_FROM"], cfg.get("TRAIN.FINETUNE_STRICT_LOAD", True)
            )
    ...
    def train(self, cfg: Dict) -> None:
        ...
        for epoch_index in range(self.start_epoch, self.num_epochs):
            ...
            if epoch < self.num_epochs:
                self.logger.info(
                    "The estimated training finish time is {}".format(
                        time.strftime(
                            "%Y-%m-%d %H:%M:%S", time.localtime(expected_end_time)
                        )
                    )
                )
        ...
        self.logger.info(
            "The training finished at {}".format(
    ...
    @torch.no_grad()
    @master_only
    def Test4OOM(self, cfg: Dict = None, train_epoch: Optional[int] = None):
        if train_epoch is None:
            self.test_data_loader = self.build_test_data_loader(cfg)
            self.register_epoch_meter("test/time", "test", "{:.2f} (s)", plt=False)
            self.register_epoch_meter("test/loss", "test", "{:.4f}", plt=False)
            self.register_epoch_meter("test/MAE", "test", "{:.4f}", plt=False)
            self.register_epoch_meter("test/MAPE", "test", "{:.4f}", plt=False)
            self.register_epoch_meter("test/RMSE", "test", "{:.4f}", plt=False)
        self.on_test_start()
    ...
    def test_pipeline(
        ...
        if save_results:
            self.logger.info(
                f'Test results saved to {os.path.join(self.ckpt_save_dir, "test_results.npz")}.'
            )
        if save_metrics:
            self.logger.info(
                f'Test metrics saved to {os.path.join(self.ckpt_save_dir, "test_metrics.json")}.'
            )
        ...
        if hasattr(cfg, "TEST"):
            ...
            best_model_path = os.path.join(
                ...
                "{}_best_val_{}.pt".format(
                    self.model_name, self.target_metrics.replace("/", "_")
                ),
            )
        ...
        try:
            ...
            if isinstance(self.model, DDP):
                self.model.module.load_state_dict(
                    checkpoint_dict["model_state_dict"], strict=strict
                )
            else:
    ...
    def backward(self, loss: torch.Tensor):
        ...
        if self.clip_grad_param is not None:
            torch.nn.utils.clip_grad_norm_(
                self.model.parameters(), **self.clip_grad_param
            )
        self.optim.step()
