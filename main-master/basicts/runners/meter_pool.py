...
class MyMeterPool:
    ...
    def get_avg(self, name: str) -> float:
        ...
        if "RMSE" in name:
            # print('@@@RMSE',sqrt(self._pool[name]['meter'].avg))
            return sqrt(self._pool[name]['meter'].avg)
            
        # APE AE is ok
        # print('@@@other',self._pool[name]['meter'].avg)
        return self._pool[name]['meter'].avg
        ...
        for i in range(len(self._pool.keys())):
            for name, value in self._pool.items():
                if value['index'] == i and value['type'] == meter_type:
                    ...
                    #     )
                    print_list.append(
                        ('{}: ' + value['format']).format(name, self.get_avg(name))
                    )
    ...
    def plt_meters(self, meter_type: str, step: int, tensorboard_writer: SummaryWriter):
        ...
        for name, value in self._pool.items():
            if value['plt'] and value['type'] == meter_type:
                tensorboard_writer.add_scalar(name, self.get_avg(name), global_step=step)
...
class MeterPoolDDP(MyMeterPool):
    """MeterPoolDDP
    """
