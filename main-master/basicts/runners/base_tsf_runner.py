...
class BaseTimeSeriesForecastingRunner(BaseEpochRunner):
    ...
    def __init__(self, cfg: Dict):
        ...
        assert (
            self.target_metrics in self.metrics or self.target_metrics == "loss"
        ), f"Target metric {self.target_metrics} not found in metrics."
        assert self.metrics_best in [
            "min",
            "max",
        ], f"Invalid best metric {self.metrics_best}."
        ...
        self.evaluation_horizons = [
            _ - 1 for _ in cfg.get("EVAL", EasyDict()).get("HORIZONS", [])
        ]
        assert (
            len(self.evaluation_horizons) == 0 or min(self.evaluation_horizons) >= 0
        ), "The horizon should start counting from 1."
    ...
    def setup_graph(self, cfg: Dict, train: bool):
        ...
        dataloader = (
            self.build_test_data_loader(cfg=cfg)
            if not train
            else self.build_train_data_loader(cfg=cfg)
        )
        ...
        if "DATASET" not in cfg:
            ...
            if (
                ...
                in inspect.signature(
                    cfg["TRAIN"]["DATA"]["DATASET"]["TYPE"].__init__
                ).parameters
            ):
            ...
            dataset = cfg["TRAIN"]["DATA"]["DATASET"]["TYPE"](
                **cfg["TRAIN"]["DATA"]["DATASET"]["PARAM"]
            )
    ...
    def build_val_dataset(self, cfg: Dict):
        ...
        if "DATASET" not in cfg:
            ...
            if (
                "logger"
                in inspect.signature(
                    cfg["VAL"]["DATA"]["DATASET"]["TYPE"].__init__
                ).parameters
            ):
            ...
            dataset = cfg["VAL"]["DATA"]["DATASET"]["TYPE"](
                **cfg["VAL"]["DATA"]["DATASET"]["PARAM"]
            )
            self.logger.info(f"Validation dataset length: {len(dataset)}")
        else:
            dataset = cfg["DATASET"]["TYPE"](
                mode="valid", logger=self.logger, **cfg["DATASET"]["PARAM"]
            )
            self.logger.info(f"Validation dataset length: {len(dataset)}")
    ...
    def build_test_dataset(self, cfg: Dict):
        ...
        if "DATASET" not in cfg:
            ...
            if (
                "logger"
                in inspect.signature(
                    cfg["TEST"]["DATA"]["DATASET"]["TYPE"].__init__
                ).parameters
            ):
            ...
            dataset = cfg["TEST"]["DATA"]["DATASET"]["TYPE"](
                **cfg["TEST"]["DATA"]["DATASET"]["PARAM"]
            )
            self.logger.info(f"Test dataset length: {len(dataset)}")
        else:
            dataset = cfg["DATASET"]["TYPE"](
                mode="test", logger=self.logger, **cfg["DATASET"]["PARAM"]
            )
            self.logger.info(f"Test dataset length: {len(dataset)}")
    ...
    def curriculum_learning(self, epoch: int = None) -> int:
        ...
        if epoch < self.warm_up_epochs:
            ...
            progress = (
                (epoch - self.warm_up_epochs) // self.cl_epochs + 1
            ) * self.cl_step_size
    ...
    def metric_forward(self, metric_func, args: Dict) -> torch.Tensor:
        ...
        if isinstance(metric_func, functools.partial):
            if (
                "null_val" not in metric_func.keywords and "null_val" in covariate_names
            ):  # null_val is required but not provided
    ...
    def train_iters(
        self, epoch: int, iter_index: int, data: Union[torch.Tensor, Tuple]
    ) -> torch.Tensor:
        ...
        forward_return = self.forward(
            data=data, epoch=epoch, iter_num=iter_num, train=True
        )

        if self.cl_param:
            cl_length = self.curriculum_learning(epoch=epoch)
            forward_return["prediction"] = forward_return["prediction"][
                :, :cl_length, :, :
            ]
        ...
        for metric_name, metric_func in self.metrics.items(): 
            ...
            self.update_epoch_meter(f"train/{metric_name}", value_item.item(),num_item.item())
    ...
    def val_iters(self, iter_index: int, data: Union[torch.Tensor, Tuple]):
        ...
        forward_return = self.forward(
            data=data, epoch=None, iter_num=iter_index, train=False
        )
        loss = self.metric_forward(self.loss, forward_return)
        ...
        for metric_name, metric_func in self.metrics.items():
            ...
            self.update_epoch_meter(f"val/{metric_name}", value_item.item(), num_item.item())  

    def test_iters_4OOM(self, iter_index: int, data: Union[torch.Tensor, Tuple]):
        forward_return = self.forward(
            data=data, epoch=None, iter_num=iter_index, train=False
        )
        loss = self.metric_forward(self.loss, forward_return)
        ...
        for data in tqdm(self.test_data_loader):
            ...
            if not self.if_evaluate_on_gpu:
                forward_return["prediction"] = (
                    forward_return["prediction"].detach().cpu()
                )
    ...
    def on_validating_end(self, train_epoch: Optional[int]):
        ...
        if train_epoch is not None:
            self.save_best_model(
                train_epoch, "val/" + self.target_metrics, greater_best=greater_best
            )
