...
def masked_rmse_bts(prediction: torch.Tensor, target: torch.Tensor, null_val: float = np.nan) -> torch.Tensor:
    """
    ...
    return torch.sqrt(masked_mse_bts(prediction=prediction, target=target, null_val=null_val))
